...
<head>
    ...
    <style>
        ...
        @media(max-width:1100px) { .detail-grid { grid-template-columns: 1fr 350px; gap: 1.5rem; } }
        @media(max-width:992px) { .detail-grid { grid-template-columns: 1fr; } }
        .amenity-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 0.75rem; margin-top: 1rem; }
        @media(max-width:576px) { .amenity-grid { grid-template-columns: 1fr 1fr; } }
        .slot-choice { display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 0.75rem; }
        .slot-btn { width: 100%; padding: 0.85rem; border: 2px solid var(--border); border-radius: var(--radius); background: white; cursor: pointer; text-align: center; transition: var(--transition); font-size: 0.85rem; font-weight: 600; }
        .slot-btn:hover, .slot-btn.active { border-color: var(--primary); background: var(--primary-light); color: var(--primary); }
        .slot-btn.active { border-color: var(--primary); box-shadow: 0 4px 12px rgba(217, 119, 6, 0.15); }
        .price-breakdown { background: #f8fafc; border-radius: var(--radius); padding: 1.25rem; border: 1px solid var(--border); }
        .price-row { display: flex; justify-content: space-between; align-items: center; padding: 0.5rem 0; font-size: 0.875rem; }
        .price-row.total { font-weight: 700; font-size: 1rem; border-top: 2px solid var(--border); padding-top: 0.75rem; margin-top: 0.25rem; }
        .month-nav { display: flex; align-items: center; gap: 1rem; }
        .month-nav a { width: 36px; height: 36px; border-radius: 50%; border: 1px solid var(--border); display: flex; align-items: center; justify-content: center; color: var(--gray); transition: var(--transition); font-size: 0.85rem; }
        .month-nav a:hover { background: var(--primary); color: white; border-color: var(--primary); }
    </style>
</head>
<body>
    <!-- SHARED NAVBAR -->
            ...
            <div>
                ...
                <div class="card" style="margin-top:1.25rem;padding:1.75rem;border-radius:var(--radius-lg);">
                    ...
                    <div class="amenity-grid">
                        <?php
                        $amenity_icons = [
                            'AC' => 'fas fa-snowflake', 'Air Conditioning' => 'fas fa-snowflake',
                            'Parking' => 'fas fa-parking', 'Catering' => 'fas fa-utensils',
                            'Stage' => 'fas fa-theater-masks', 'Generator' => 'fas fa-bolt',
                            'WiFi' => 'fas fa-wifi', 'Music' => 'fas fa-music',
                            'Decoration' => 'fas fa-flower', 'CCTV' => 'fas fa-camera',
                            'Kitchen' => 'fas fa-kitchen-set', 'Restrooms' => 'fas fa-restroom',
                            'Projector' => 'fas fa-film', 'Lift' => 'fas fa-elevator',
                        ];
                        foreach ($facilities as $fac):
                            $icon = 'fas fa-check-circle';
                            foreach ($amenity_icons as $key => $ico) {
                                if (stripos($fac, $key) !== false) { $icon = $ico; break; }
                            }
                        ?>
